...
extern void vita2d_texture_set_pixel(vita2d_texture* t, int x, int y, unsigned int c);
...
void tickTouchMap(){
    if(shouldRenderMap){
        if(k_touch.x > 0 || k_touch.y > 0){
            // Plus/Minus zoom button
            if(k_touch.y > 204 && k_touch.y < 232){
                if(k_touch.x > 284 && k_touch.x < 312){
                    ...
                    return;
                } else if(k_touch.x > 256 && k_touch.x < 284){
                    if(zoomLevel < 4) return;
                    if(!isChangingSize && !isDraggingMap){
                        mScrollX += (50 * (zoomLevel/2));
                        mScrollY += (40 * (zoomLevel/2));
            ...
            } else if(k_touch.y > 8 && k_touch.y < 40 && k_touch.x > 284 && k_touch.x < 312){
                // Exit Button
                if(!isChangingSize && !isDraggingMap){
            ...
            if(!isDraggingMap){
                lastTouchX = k_touch.x;
                lastTouchY = k_touch.y;    
            }
            if(zoomLevel > 2){
                int dx = lastTouchX - k_touch.x;
                if(dx > 1 || dx < -1){
                    mScrollX -= dx;
                    if(mScrollX < 320-(128*zoomLevel)) mScrollX = 320-(128*zoomLevel);
                    else if(mScrollX > 0) mScrollX = 0;
        ...
        if(k_touch.y > 100 && k_touch.y < 228 && k_touch.x > 96 && k_touch.x < 228){
            shouldRenderMap = true;
        }
    }
}

void hurtEntity(Entity* e, int damage, int dir, uint32_t hurtColor){
...
void setTile(int id, int x, int y){
    ...
    vita2d_texture_set_pixel(minimap[currentLevel], x, y, getTileColor(id));
}
...
void playerAttack(){
	...
	if (xt >= 0 && yt >= 0 && xt < 128 && yt < 128) {
        int8_t itract = itemTileInteract(getTile(xt,yt),player.p.activeItem,xt,yt,player.x,player.y,player.p.dir);
        if(itract > 0){
...
void switchLevel(int8_t change){
    ...
    if(currentLevel == 1) vita2d_set_clear_color(RGBA8(0x82, 0x6D, 0x6C, 0xFF));
    else if(currentLevel > 1) vita2d_set_clear_color(RGBA8(0x66, 0x66, 0x66, 0xFF));
    else vita2d_set_clear_color(RGBA8(0x00, 0x7F, 0x00, 0xFF));
}
