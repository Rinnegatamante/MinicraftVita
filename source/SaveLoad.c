...
int16_t calculateImportantEntites(EntityManager * eManager, int level){
    int i;
    int16_t count = 0;
...
void saveCurrentWorld(char * filename, EntityManager * eManager, Entity * player, uint8_t * map, uint8_t * mapData){
    ...
    fwrite(&player->p.health, sizeof(int16_t), 1, file);
    fwrite(&player->x, sizeof(int16_t), 1, file);
    fwrite(&player->y, sizeof(int16_t), 1, file);
    fwrite(&currentLevel, sizeof(int8_t), 1, file);
    
    // Inventory Data
    fwrite(&eManager->nextInv, sizeof(int16_t), 1, file); // write amount of inventories.
    for(i = 0; i < eManager->nextInv; ++i) {
        fwrite(&eManager->invs[i].lastSlot, sizeof(int16_t), 1, file); // write amount of items in inventory;
        for(j = 0; j < eManager->invs[i].lastSlot; ++j) {
            fwrite(&eManager->invs[i].items[j].id, sizeof(int16_t), 1, file); // write ID of item
            fwrite(&eManager->invs[i].items[j].countLevel, sizeof(int16_t), 1, file); // write count/level of item
    ...
    for(i = 0; i < 5; ++i){
        ...
        fwrite(&amount, sizeof(int16_t), 1, file); // read amount of entities in level.
        for(j = 0; j < eManager->lastSlot[i]; ++j){
            if(!entityIsImportant(&eManager->entities[i][j])) continue;
            fwrite(&eManager->entities[i][j].type, sizeof(int16_t), 1, file); // write entity's type ID
            fwrite(&eManager->entities[i][j].x, sizeof(int16_t), 1, file); // write entity's x coordinate
            fwrite(&eManager->entities[i][j].y, sizeof(int16_t), 1, file); // write entity's y coordinate
            switch(eManager->entities[i][j].type){
                case ENTITY_AIRWIZARD:
                    fwrite(&eManager->entities[i][j].wizard.health, sizeof(int16_t), 1, file);
                    break;
                case ENTITY_SLIME:
                    fwrite(&eManager->entities[i][j].slime.health, sizeof(int16_t), 1, file);
                    fwrite(&eManager->entities[i][j].slime.lvl, sizeof(int8_t), 1, file);
                    break;
                case ENTITY_ZOMBIE:
                    fwrite(&eManager->entities[i][j].zombie.health, sizeof(int16_t), 1, file);
                    fwrite(&eManager->entities[i][j].zombie.lvl, sizeof(int8_t), 1, file);
                    break;
                case ENTITY_ITEM:
                    fwrite(&eManager->entities[i][j].entityItem.item.id, sizeof(int16_t), 1, file);
                    fwrite(&eManager->entities[i][j].entityItem.item.countLevel, sizeof(int16_t), 1, file);
                    fwrite(&eManager->entities[i][j].entityItem.age, sizeof(int16_t), 1, file);
                    break;
                case ENTITY_FURNITURE:
                    fwrite(&eManager->entities[i][j].entityFurniture.itemID, sizeof(int16_t), 1, file);
    ...
    // Map Data
    fwrite(map, sizeof(uint8_t), 128*128*5, file); // Map Tile IDs, 128*128*5 bytes = 80KB
    fwrite(mapData, sizeof(uint8_t), 128*128*5, file); // Map Tile Data (Damage done to trees/rocks, age of wheat & saplings, etc). 80KB
...
int loadWorld(char * filename, EntityManager * eManager, Entity * player, uint8_t * map, uint8_t * mapData){
    ...
    int i,j;
        ...
        if ((file = fopen(filename, "rb"))){
            ...
            fread(&player->p.health, sizeof(int16_t), 1, file);
            fread(&player->x, sizeof(int16_t), 1, file);
            fread(&player->y, sizeof(int16_t), 1, file);
            fread(&currentLevel, sizeof(int8_t), 1, file);
            
            fread(&eManager->nextInv, sizeof(int16_t), 1, file);
            for(i = 0; i < eManager->nextInv; ++i) {
                fread(&eManager->invs[i].lastSlot, sizeof(int16_t), 1, file); // read amount of items in inventory;
                for(j = 0; j < eManager->invs[i].lastSlot; ++j) {
                    fread(&eManager->invs[i].items[j].id, sizeof(int16_t), 1, file); // write ID of item
                    fread(&eManager->invs[i].items[j].countLevel, sizeof(int16_t), 1, file); // write count/level of item
            ...
            for(i = 0; i < 5; ++i){
                fread(&eManager->lastSlot[i], sizeof(int16_t), 1, file); // read amount of entities in level.
                for(j = 0; j < eManager->lastSlot[i]; ++j){
                    fread(&eManager->entities[i][j].type, sizeof(int16_t), 1, file); // read entity's type ID
                    fread(&eManager->entities[i][j].x, sizeof(int16_t), 1, file); // read entity's x coordinate
                    fread(&eManager->entities[i][j].y, sizeof(int16_t), 1, file); // read entity's y coordinate
                    ...
                    switch(eManager->entities[i][j].type){
                        ...
                        case ENTITY_SPARK:
                            ...
                            break;
                        case ENTITY_AIRWIZARD:
                            fread(&eManager->entities[i][j].wizard.health, sizeof(int16_t), 1, file);
                            eManager->entities[i][j].level = i;
                            ...
                            break;
                        case ENTITY_SLIME:
                            fread(&eManager->entities[i][j].slime.health, sizeof(int16_t), 1, file);
                            fread(&eManager->entities[i][j].slime.lvl, sizeof(int8_t), 1, file);
                            eManager->entities[i][j].level = i;
                            eManager->entities[i][j].hurtTime = 0;
                            eManager->entities[i][j].xKnockback = 0;
                            eManager->entities[i][j].yKnockback = 0;
                            ...
                            break;
                        case ENTITY_ZOMBIE:
                            fread(&eManager->entities[i][j].zombie.health, sizeof(int16_t), 1, file);
                            fread(&eManager->entities[i][j].zombie.lvl, sizeof(int8_t), 1, file);
                            eManager->entities[i][j].level = i;
                            eManager->entities[i][j].hurtTime = 0;
                            eManager->entities[i][j].xKnockback = 0;
                            eManager->entities[i][j].yKnockback = 0;
                        ...
                        case ENTITY_ITEM:
                            ...
                            fread(&eManager->entities[i][j].entityItem.item.id, sizeof(int16_t), 1, file);
                            fread(&eManager->entities[i][j].entityItem.item.countLevel, sizeof(int16_t), 1, file);
                            fread(&eManager->entities[i][j].entityItem.age, sizeof(int16_t), 1, file);
                            eManager->entities[i][j].level = i;
                            ...
                            break;
                        case ENTITY_FURNITURE:
                            fread(&eManager->entities[i][j].entityFurniture.itemID, sizeof(int16_t), 1, file);
                            int invIndex;
                            fread(&invIndex, sizeof(int), 1, file);
            ...
            fread(map, sizeof(uint8_t), 128*128*5, file);
            fread(mapData, sizeof(uint8_t), 128*128*5, file);
            fclose(file);
            return 0;
        }
    return 1;
